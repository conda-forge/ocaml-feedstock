...
export OCAML_PREFIX=${SRC_DIR}/_cross && mkdir -p ${SRC_DIR}/_cross
...
# CRITICAL: _native/bin MUST come FIRST to ensure ocamlc matches source version being built
# - _native/bin has 5.4.0 ocamlc/ocamlyacc/ocamlrun/ocamllex built in Stage 1
# - BUILD_PREFIX/bin has 5.3.x ocamlc from build dependency (version mismatch!)
# - _cross/bin has target-arch cross-compiled ocamlyacc that won't run on BUILD machine
export PATH="${SRC_DIR}/_native/bin:${BUILD_PREFIX}/bin:${OCAML_PREFIX}/bin:${_PATH}"
export OCAMLLIB=$OCAML_PREFIX/lib/ocaml
