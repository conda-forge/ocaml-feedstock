...
clean_runtime_launch_info() {
  ...
  # DISABLED: Binary file manipulation is too fragile and corrupts the file
  # Conda's relocation mechanism should handle placeholder paths in the final package
  # The runtime-launch-info file has embedded binary data (ocamlrun) that breaks if modified
  echo "  [SKIP] runtime-launch-info cleanup disabled (binary file)"
  return 0

  # runtime-launch-info is a BINARY file:
  # Line 1: "sh"
  # Line 2: PREFIX path
  # Line 3+: ELF binary (ocamlrun)
  # We MUST NOT use sed on the entire file (corrupts binary)

  # Extract first line (sh)
  local line1
  line1=$(head -1 "${runtime_launch_info}")

  # Extract second line (path) - this is what we need to clean
  local line2_original
  line2_original=$(head -2 "${runtime_launch_info}" | tail -1)

  # CRITICAL: Calculate header size based on ORIGINAL line2 length
  # This tells us where the binary starts in the original file
  local header_size
  header_size=$((${#line1} + 1 + ${#line2_original} + 1))  # +1 for each newline

  # Check if line2 contains build-time paths that need cleaning
  if ! echo "${line2_original}" | grep -qE "rattler-build|conda-bld|build_env|placehold|host_env|feedstock"; then
    # No cleaning needed
    return 0
  fi

  # Apply path cleaning to line2
  local line2_cleaned="${line2_original}"
  line2_cleaned=$(echo "${line2_cleaned}" | sed 's|[^[:space:]]*/bld/rattler-build_[^[:space:]]*|'"${prefix}"'|g')
  line2_cleaned=$(echo "${line2_cleaned}" | sed 's|[^[:space:]]*/output/bld/[^[:space:]]*|'"${prefix}"'|g')
  line2_cleaned=$(echo "${line2_cleaned}" | sed 's|[^[:space:]]*host_env_placehold[^[:space:]]*|'"${prefix}"'|g')
  line2_cleaned=$(echo "${line2_cleaned}" | sed 's|[^[:space:]]*build_env_placehold[^[:space:]]*|'"${prefix}"'|g')
  line2_cleaned=$(echo "${line2_cleaned}" | sed 's|[^[:space:]]*rattler-build[^[:space:]]*|'"${prefix}"'|g')
  line2_cleaned=$(echo "${line2_cleaned}" | sed 's|[^[:space:]]*conda-bld[^[:space:]]*|'"${prefix}"'|g')
  line2_cleaned=$(echo "${line2_cleaned}" | sed 's|/home/[^/]*/feedstock_root[^[:space:]]*|'"${prefix}"'|g')

  # CRITICAL: Pad line2 to match original length
  # The binary starts at a fixed offset - changing line2 length corrupts the file
  local original_len=${#line2_original}
  local cleaned_len=${#line2_cleaned}
  if [[ $cleaned_len -lt $original_len ]]; then
    # Pad with spaces to match original length
    local padding_needed=$((original_len - cleaned_len))
    line2_cleaned="${line2_cleaned}$(printf '%*s' $padding_needed '')"
  elif [[ $cleaned_len -gt $original_len ]]; then
    # Truncate if somehow longer (shouldn't happen, but safety)
    line2_cleaned="${line2_cleaned:0:$original_len}"
  fi

  # Extract binary portion using ORIGINAL header size (where binary starts in original file)
  local temp_binary="${runtime_launch_info}.binary.tmp"
  # Use byte-based extraction ONLY (line-based corrupts binary data)
  if ! tail -c +$((header_size + 1)) "${runtime_launch_info}" > "${temp_binary}" 2>/dev/null; then
    echo "ERROR: Failed to extract binary from runtime-launch-info" >&2
    return 1
  fi

  # Reassemble file: text header + binary
  # Use printf -n to avoid extra newlines
  {
    printf '%s\n' "${line1}"
    printf '%s\n' "${line2_cleaned}"
    cat "${temp_binary}"
  } > "${runtime_launch_info}.new"

  mv "${runtime_launch_info}.new" "${runtime_launch_info}"
  rm -f "${temp_binary}"
}
